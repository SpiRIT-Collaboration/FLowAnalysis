...
// DoAna.h
// Header for analysis coded by Mizuki
//
...
PlotStyle PStyle[] = { {0,kRed,       kFullCircle,      1,    "Proton"},
		       ...
		       {5,kGreen-3,   kFullTriangleDown,1.5,  "H"},
		       {6,kGreen-3,   kFullStar,        1.5,  "HHe"},
		       {7,kGreen-3,   3,                1.5,  "n"},
		       {8,kRed,       24,               1,    "Proton"},
		       {9,kBlue,      25,               1,    "Deuteron"},
		       {10,kOrange+1, 26,               1.5,  "Triton"},
		       {11,kViolet-1, 27,               1.5,  "3He"},
		       {12,kGreen-3,  28,               1.5,  "4He"},
		       {13,kGreen-3,  kOpenTriangleDown,1.5,  "H"},
		       {14,kGreen-3,  kOpenStar,        1.5,  "HHe"},
		       {16,kGreen-3,  3,                1.5,  "n"}};
...
Double_t mass[]    = {938.2720813, 1875.612762, 2808.921112, 2808.39132, 3727.379378, 938.2720813, 938.2720813, 939.5654};
...
nucleus  ncls[] = {
  {0,      2212, 1.,   1.,   938.272081, "proton"  ,"Proton",  "1H"  ,"^{1}H"},
  {1,1000010020, 2.,   1.,  1875.612762, "deuteron","Deuteron","2H"  ,"^{2}H"},
  {2,1000010030, 3.,   1.,  2808.921112, "triton",  "Triton",  "3H"  ,"^{3}H"},
  {3,1000020030, 3.,   2.,  2808.39132,  "3He",     "3He",     "3He" ,"^{3}He"}, 
  {4,1000020020, 4.,   2.,  3727.379378, "4He",     "4He",     "4He" ,"^{4}He"}, 
  {5,      2212, 1.,   1.,   938.272081, "H",       "H",       "H"   ,"H"}, 
  {6,      2212, 1.,   1.,   938.272081, "HHe",     "HHe",     "HHe" ,"HHe"}, 
  {7,      2112, 1.,   0.,   939.5654,   "neutron", "Neutron", "n"   ,"n"},
  {8,       211, 0.15,-1.,   139.57061,  "pi-",     "Pi-",     "Pn"  ,"#pi^{-}"},  
  {9,       211, 0.15, 1.,   139.57061,  "pi+",     "Pi+",     "Pn"  ,"#pi^{+}"},  
};
...
TFile* GetAcceptanceCorrectionFile(UInt_t mtconfig=0, UInt_t phicutID=5, TString comment="" )
{
  ...
  TString fname;
  if( mtconfig == 1 )
    //    fname = "data/rootfiles/UnfoldedLCPSpectra_-30to20_46to55_embed108_left.rnd_vapri.slim.root";
    fname = "data/rootfiles/UnfoldedLCPSpectra_-30to20_46to55_embed108_left1rad_vapri.slim.root";
  else
    //    fname = "data/rootfiles/UnfoldedLCPSpectra_-30to20_55to80_embed108_left.rnd_vapri.slim.root";
    fname = "data/rootfiles/UnfoldedLCPSpectra_-30to20_55to80_embed108_left1rad_vapri.slim.root";
...
void SetXTextLabel(TMultiGraph& obj, std::vector< UInt_t > spart )
{
  if( slabel.size() < 1 ) {
    for( auto i : spart ) {
      slabel.push_back( ncls[i].pLabel ); 
    }
  }
  //  cout << slabel 
  
  obj.GetXaxis()->SetLimits(0.5,slabel.size()+0.5);
  //  obj.GetXaxis()->SetNdivision(510);
  ...
  obj.GetXaxis()->SetLabelSize(20);
  obj.GetXaxis()->LabelsOption("v");
  //  obj.GetXaxis()->SetLabelOffset(0.1);
}
