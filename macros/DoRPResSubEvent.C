...
Double_t *GetRPResolutionwChi(UInt_t vsel, Double_t chi, Double_t chie, Double_t mr, Double_t mre2);
//--


//-------------------//
void DoRPResSubEvent(Int_t isel = 0) 
{
  gROOT->Reset();
...
void GetResolution()
{
  ...
  TH1D *h2phi0_180  = new TH1D("h2phi0_180" , "0to180",100,0.,3.2);
  TH1D *h2phi90_180 = new TH1D("h2phi90_180","90to180",100,0.,3.2);


  TH1D *hdphi1  = new TH1D("hdphi1",";#Delta(#Psi)",100, -TMath::Pi(), TMath::Pi());
  TH1D *hdphi2  = new TH1D("hdphi2",";#Delta(2#Psi)",100, -TMath::Pi(), TMath::Pi());

  TH1D *hcos1   = new TH1D("hcos1" ,";<cos(#Delta#Psi)>" , 100,-1.,1.);
  TH1D *hcos2   = new TH1D("hcos2" ,";<cos(2#Delta#Psi)>", 100,-1.,1.);
  ...
  for(Int_t i = 0; i < nevt; i++){
    ...
    hcos1 -> Fill( aflow->cosdPsi );
    hcos2 -> Fill( aflow->cos2dPsi );
  ...
  LOG(INFO) << " <cos (d phi) > = "  << *reaolution << " +- "     << *(reaolution+1) << FairLogger::endl;
  LOG(INFO) << " <cos (2d phi) > = " << *(reaolution+2) << " +- " << *(reaolution+3) << FairLogger::endl;

  auto reaolution2 = GetRPResolutionwChi(h2phi0_180, h2phi90_180);
  LOG(INFO) << "2Psi: <cos (2d phi) > = " << *(reaolution2+2) << " +- " << *(reaolution2+3) << FairLogger::endl;

  Double_t rescos1 = sqrt(2.*hcos1->GetMean());
  Double_t rescos2 = sqrt(2.*hcos2->GetMean());

  LOG(INFO) << " <cos (Psi_m -Psi_r) > = " << rescos1 << FairLogger::endl;
  LOG(INFO) << " <cos2(Psi_m -Psi_r) > = " << rescos2 << FairLogger::endl;
  ...
  cc->cd(2);
  h2phi0_180->Draw();
  h2phi90_180->SetLineColor(2);
  h2phi90_180->Draw("same");
...
void PsiAngleDependence()            //%% Executable :
{
  ...
  for(UInt_t k = 0; k < npsi; k++){
    TString htitle = Form("hcosdphi_%d",k);
    hcosdphi[k]  = new TH1D(htitle, "",100,-1., 1.);
    hpsi[k] = new TH1D(Form("hpsi%d",k),"",100,-3.15,3.15);

    htitle = Form("h2cosdphi_%d",k);
    h2cosdphi[k]  = new TH1D(htitle, "",100,-1.,1.);
  }
  ...
  for(Int_t i = 0; i < nEntry; i++){
    ...
    hcosdphi[iphi] ->Fill(aflow->cosdPsi);
    h2cosdphi[iphi] ->Fill(aflow->cos2dPsi);
  }
  ...
  for(UInt_t j = 0; j < 2; j++) {
    for(UInt_t i = nst[j]; i < nst[j]+6; i++) {
      ...
      if( hcosdphi[i]->GetEntries() < 5 ) continue;

      Double_t psi = hpsi[i]->GetMean();

      Double_t rescos1 = sqrt(2.*hcosdphi[i]->GetMean());
      Double_t rescos2 = sqrt(2.*h2cosdphi[i]->GetMean());

      gv_psi1->SetPoint(ip, psi, rescos1);
      gv_psi1->SetPointError(ip, 0., 0.);
      
      //      rpres = GetRPResolutionwChi(h2cosdphi[i], h2phi90_180[i]);

      gv_psi2->SetPoint(ip, psi, rescos2);
      gv_psi2->SetPointError(ip, 0., 0.);
      ip++;
...
void CentralityDependence()            //%% Executable :
{
  ...
  TString fName = "data/smlt_"+ sysName + ".v"+ oVer +".root";
  ...
  for(UInt_t k = 0; k < mbin; k++){
    ...
    htitle = Form("hcosdphi_%d",k);
    hcosdphi[k]  = new TH1D(htitle,"",100,0.,3.2);
    htitle = Form("hphi90_180_%d",k);
    hphi90_180[k] = new TH1D(htitle,"",100,0.,3.2);

    htitle = Form("h2cosdphi_%d",k);
    h2cosdphi[k]  = new TH1D(htitle,"",100,0.,3.2);
  ...
  for(UInt_t i = 0; i < mbin; i++) {
    ...
    rpres = GetRPResolutionwChi(hcosdphi[i], hphi90_180[i]);
...
TString GetPsiRPFileName()
{
  TString fn = "data/spsi_"+ sysName + ".v" + oVer;
...
Double_t *GetRPResolutionwChi(TH1D *hphi0_180, TH1D *hphi90_180)            //%% Executable : 
{
  ...
  LOG(INFO) << " m0 : " << m0 << " m1 " << m1 << FairLogger::endl;
  ...
  Double_t *rp    = new Double_t[2];
  rp      = GetRPResolutionwChi(1, chi, chie, mr, mre2);
  rpres[0] = rp[0];
  rpres[1] = rp[1];

  rp     = GetRPResolutionwChi(2, chi, chie, mr, mre2);
  rpres[2] = rp[0];
  rpres[3] = rp[1];

  LOG(DEBUG) << " Getting correction factor " 
	     << std::setw(6)  << mr << " = " << mre2 << " chi = " << chi 
	     << " v1 "
...
Double_t *GetRPResolutionwChi(UInt_t vsel, Double_t chi, Double_t chie, Double_t mr, Double_t mre2)
{
  ...
  if( vsel == 1 ) {
    ...
    Double_t err2     = mre2*pow( par2[0] + 2.*par2[1]*chi + 4.*par2[2]*pow(chi,3) + 5.*par2[3]*pow(chi,4),2);
    rpres[1] = sqrt( err2 );
  }
  ...
  return rpres;
} //Double_t *GetRPResolutionwChi(Double_t chi, Double_t chie)
