...
Double_t sysBN[]   = {82./50., 58./50.,   74./50.,62./50.,  50./50.};
Double_t sysN[]    = {156.,    110.,      136.,   136.   ,  156.};
Double_t sysNA[]   = {156./100.,110./100.,136./100.,136./100.,  156./100.};
...
void PlotFigure()
{
  ...
  lrv1 = new TLegend(0.4 , 0.20, 0.70, 0.40, "");
  lrv2 = new TLegend(0.5 , 0.60, 0.70, 0.9, "");
  ...
  for( UInt_t is = 0; is < 4; is++ ) {
    TString otitle = lsys[is] + gnames[0].comment;
	...
	LOG(INFO) << fname << " is opened. " << FairLogger::endl;
      ...
      if( yv1 && yv2 ) {
	yv1->SetMarkerColor(icol[is]+ip);
	yv1->SetMarkerStyle(imark[is]);
	yv1->SetLineColor(icol[is]+ip);

	yv2->SetMarkerColor(icol[is]+ip);
  ...
  for( UInt_t ip = 0; ip < 4; ip++ ){
    ...
    g_v2max[ip]  -> SetTitle(";"+xlabel+"; -v2 max");
...
void Draw_v1v2SystemD()
{
  ...
  mrv1->Draw("ALP");
  auto aLineX1 = new TLine( mrv1->GetXaxis()->GetXmin(), 0.,
			    mrv1->GetXaxis()->GetXmax(), 0.);
...
void Draw_Indiv_v1SystemD() 
{
  ...
  Double_t BYs = 0.1;            // Bottom Y space
  Double_t TYm = 0.02;            // Top Y mergin
  Double_t BYm = 0.1;            // Bottom Y mergin
